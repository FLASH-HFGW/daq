...
// macro to silence the "unreferenced parameter" in those cases where the linux
// code needs a function parameter while the windows code does not
...
void vGetIPAndSubnetMask (int32 lNICIdx, SOCKET lSockfd, char* szIPAddress, char* szSubNetMask)
    ...
    if (dwErr != ERROR_SUCCESS) // dwErr will also be set if the PC does not have any adapters (ERROR_NO_DATA)
        {
        if (bDelete)
            free (pstFirstAdapterInfo);
        return;
        }
    if (pstFirstAdapterInfo == NULL) // I don't think this can happen, but the VS code analysis always complains about unhandled NULL access otherwise
        return;
...
char* szNetworkErrorMessage (int32 lErrorCode)
    ...
    // convert to char*
    int lWritten = _snprintf_s (s_szErrMsgBuffer, 100, _TRUNCATE, "%s", szErrorText);

    // Avoid double CR at log file.
    if ((lWritten >= 2) &&
        (s_szErrMsgBuffer[lWritten - 2] == 0x0d) && (s_szErrMsgBuffer[lWritten - 1] == 0x0a))
        s_szErrMsgBuffer[lWritten - 2] = 0x00;
...
char* szIPFromName (const char* szName, int* plFamily)
    {
    static char szIPBuffer[46]; // large enough for IPv6 address
    ...
    struct sockaddr* pstSockAddr = pstResult->ai_addr;

    // Convert the binary IP address into a readable string.
    if (lInet_ntop (pstResult->ai_family, pstSockAddr, szIPBuffer) == SOCKET_ERROR)
        {
        freeaddrinfo (pstResult);
        return NULL;
        }
    *plFamily = pstResult->ai_family;
    freeaddrinfo (pstResult);

    return szIPBuffer;
    }

int lInet_pton (const char* szHostname, void* pvSockAddrOrSockAddr6)
    {
...
#else
    int lFamily = 0;
    const char* szIP = szIPFromName (szHostname, &lFamily);
    if (lFamily == AF_INET)
        {
        struct sockaddr_in* pstSockAddr = (struct sockaddr_in*)pvSockAddrOrSockAddr6;
        if (inet_pton (AF_INET, szIP, &(pstSockAddr->sin_addr)) <= 0)
            {
            return SOCKET_ERROR;
            }
        }
        ...
        {
        struct sockaddr_in6* pstSockAddr6 = (struct sockaddr_in6*)pvSockAddrOrSockAddr6;
        if (inet_pton (AF_INET6, szIP, &(pstSockAddr6->sin6_addr)) <= 0)
            {
            return SOCKET_ERROR;
            }
        }
...
int lInet_ntop (int lFamily, void* pvSockAddrOrSockAddr6, char* szIP)
    {
#if defined(_MSC_VER) && (_MSC_VER < 1900) // for compilation with older VS. 1900 = VS2015
    ...
    if (lFamily == AF_INET)
        {
        struct sockaddr_in* pstSockAddr = (struct sockaddr_in*)pvSockAddrOrSockAddr6;
        if (inet_ntop (AF_INET, &(const_cast < struct sockaddr_in* > (pstSockAddr)->sin_addr), szIP, 16) == NULL)
            {
            return SOCKET_ERROR;
            }
        }
        ...
        {
        struct sockaddr_in6* pstSockAddr6 = (struct sockaddr_in6*)pvSockAddrOrSockAddr6;
        if (inet_ntop (AF_INET6, &(pstSockAddr6->sin6_addr), szIP, 64) == NULL)
            {
            return SOCKET_ERROR;
            }
        }
